...
public class DatabaseIntrospector {
    ...
    private Map<ActualTableName, List<IntrospectedColumn>> getColumns(
        ...
        // for customize
        ResultSet tableResultSet = databaseMetaData.getTables(localCatalog, localSchema, localTableName, null);
        while (tableResultSet.next()) {
    		String tableName = tableResultSet.getString("TABLE_NAME");
    		ActualTableName atn = new ActualTableName(
    				tableResultSet.getString("TABLE_CAT"), //$NON-NLS-1$
    				tableResultSet.getString("TABLE_SCHEM"), //$NON-NLS-1$
    				tableName); //$NON-NLS-1$
    		atn.setRemark(tableResultSet.getString("REMARKS"));
    		...
    		ResultSet columnResultSet = databaseMetaData.getColumns(localCatalog, localSchema,
    				tableName, null);
            ...
            while (columnResultSet.next()) {
                ...
                introspectedColumn.setJdbcType(columnResultSet.getInt("DATA_TYPE")); //$NON-NLS-1$
                introspectedColumn.setLength(columnResultSet.getInt("COLUMN_SIZE")); //$NON-NLS-1$
                introspectedColumn.setActualColumnName(columnResultSet.getString("COLUMN_NAME")); //$NON-NLS-1$
                introspectedColumn
                        .setNullable(columnResultSet.getInt("NULLABLE") == DatabaseMetaData.columnNullable); //$NON-NLS-1$
                introspectedColumn.setScale(columnResultSet.getInt("DECIMAL_DIGITS")); //$NON-NLS-1$
                introspectedColumn.setRemarks(columnResultSet.getString("REMARKS")); //$NON-NLS-1$
                introspectedColumn.setDefaultValue(columnResultSet.getString("COLUMN_DEF")); //$NON-NLS-1$
    ...
    private List<IntrospectedTable> calculateIntrospectedTables(
        ...
        for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns
            ...
            table.setIgnoreTablePrefix(tc.getIgnoreTablePrefix());
